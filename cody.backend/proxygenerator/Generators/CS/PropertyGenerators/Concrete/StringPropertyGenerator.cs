...
namespace proxygenerator.Generators.CS.PropertyGenerators.Concrete {
    public class StringPropertyGenerator: BasePropertyGenerator<StringAttributeData> {
        public StringPropertyGenerator(StringAttributeData attributeData) : base(attributeData)
        {
        }
        ...
        public override string GenerateAttributeProperties(int indentationLevel)
        {
            ...
            {
                sb.AppendLine(
                    $"{i1}set {{ this.SetPropertyValue(\"{Attribute.LogicalName}\", value, {Attribute.MaxLength}, \"{Attribute.CodeName}\"); }}");
            }
            sb.AppendLine($"{i}}}");
            return sb.ToString();
        }
