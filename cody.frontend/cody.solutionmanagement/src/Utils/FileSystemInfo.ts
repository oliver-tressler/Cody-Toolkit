...
export abstract class FileSystemInfo<T extends FileSystemInfo<T>>{
    ...
    public get name(): string { return this.parsedAbsolutePath.name; }
    public get projectRoot(): string { return this.projectRootPath; }
    public get exists(): boolean { return fs.existsSync(this.absolutePath); }
    protected get relativePath(): string {
        ...
        return this.isForwardSlash
            ? this.toForwardSlash(relativePath)
            : this.toBackwardSlash(relativePath);
    }

    constructor(absolutePath: string, projectRoot?: string, isForwardSlash?: boolean){
        this.isForwardSlash = isForwardSlash ?? true;
        this.absolutePath = isForwardSlash
            ? this.toForwardSlash(absolutePath)
            : this.toBackwardSlash(absolutePath);
        if (projectRoot == null){
