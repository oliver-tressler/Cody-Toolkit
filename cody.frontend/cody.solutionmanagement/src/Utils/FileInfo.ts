...
export class FileInfo {
	public get extension(): string { return this.parsedAbsoluteFilePath.ext; }
	public get name(): string { return this.parsedAbsoluteFilePath.name; }
	...
	public get file(): string { return this.absoluteFilePath; }
	public get relativeDir(): string { return this.isForwardSlash 
	...
	public get relativeFile(): string { return this.isForwardSlash 
	...
	public get exists(): boolean { return fs.existsSync(this.file); }
	public get workspaceRoot(): string { return this.workspacePath; }
	...
	constructor(absoluteFilePath: string, private workspacePath?: string, private isForwardSlash?: boolean) {
		...
		if (workspacePath == null) {
			this.workspacePath = this.isForwardSlash
				? this.toForwardSlash(vsc.workspace.rootPath)
				: this.toBackwardSlash(vsc.workspace.rootPath);
		} 
	...
	private toForwardSlash(string: string, escapeSpaces?: boolean): string {
		return string.replace("\\", "/").split(String.fromCharCode(92)).join('/').split("/")
			.map((dir) => (escapeSpaces ?? true) && !dir.match(/^".+"$/) && dir.includes(" ") ? "\"" + dir + "\"" : dir).join("/");
	}
