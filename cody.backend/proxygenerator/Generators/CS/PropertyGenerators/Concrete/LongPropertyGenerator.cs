...
{
    class LongPropertyGenerator: BasePropertyGenerator<NumericAttributeData>
    {
        public LongPropertyGenerator(NumericAttributeData attributeData) : base(attributeData)
        {
        }
        ...
        public override string GenerateAttributeProperties(int indentationLevel)
        {
            ...
            {
                sb.AppendLine(
                    $"{i1}set {{ this.SetPropertyValue(\"{Attribute.LogicalName}\", (long?)value, (long){Attribute.MinValue}, (long){Attribute.MaxValue}, \"{Attribute.CodeName}\"); }}");
            }
            sb.AppendLine($"{i}}}");
            return sb.ToString();
        }
