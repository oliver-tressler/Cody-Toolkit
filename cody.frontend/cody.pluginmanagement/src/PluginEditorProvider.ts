...
import { Organization, Plugin, Step, TreeData } from "./PluginTreeDataProvider";
import { withAuthentication } from "./Utils/connection";
import { WebviewInterface, WebviewRequest } from "./WebviewExtensionInterface";
type PanelInfo = {
	panelId: string;
...
export class PluginEditorProvider {
		...
		context.subscriptions.push(
			vsc.commands.registerCommand(
				...
				async (data: Organization | Plugin | Step) => {
					return await withAuthentication(async ({ activeOrganization }) => {
						...
						const webviewExtensionInterface = panelInfo.webviewInterface;
						webviewExtensionInterface.on<{ fileType: string; path: string }>(
							"plugineditor_pickfile",
							"assemblyfilepicker",
							async (message) => {
						...
						webviewExtensionInterface.on<{
							...
							FilePath: string;
						}>("plugineditor_saveassembly", "assemblysave", async (message) => {
						...
						webviewExtensionInterface.on<string>("plugineditor_savestep", "stepsave", async (message) => {
						...
						webviewExtensionInterface.on<{ EntityName: string }>(
						...
						webviewExtensionInterface.on<{ MessageId: string }>(
							"plugineditor_requeststepentities",
							"stepentities",
							async (message) => {
								try {
									return (
										await Api.retrieveEntities(activeOrganization.UniqueName, message.MessageId)
									).data;
								} catch (e) {
									vsc.window.showErrorMessage((e as AxiosError).response?.data.Message ?? e.message);
								}
						...
						webviewExtensionInterface.on<string>("plugineditor_saveimage", "imagesave", async (message) => {
							// Data is already the parent element here.
							const existingPanelDataId = this.panelIdToDataMap[panelInfo.panelId];
							if (existingPanelDataId) {
			...
			vsc.commands.registerCommand("extension.crmtooling.pluginsandsteps.edit", async (data: TreeData) => {
				return await withAuthentication(async ({ activeOrganization }) => {
					...
					const webviewExtensionInterface = new WebviewInterface(panelInfo.panel);
					webviewExtensionInterface.on<{ fileType: string; path: string }>(
						"plugineditor_pickfile",
						"assemblyfilepicker",
						async (message) => {
					...
					webviewExtensionInterface.on<{ EntityName: string }>(
						...
						async (message) => {
							try {
								return (
									await Api.retrieveStepAttributes(activeOrganization.UniqueName, message.EntityName)
								).data;
							} catch (e) {
								vsc.window.showErrorMessage((e as AxiosError).response?.data.Message ?? e.message);
							}
					...
					webviewExtensionInterface.on<{ MessageId: string }>(
						...
						async (message) => {
							try {
								return (await Api.retrieveEntities(activeOrganization.UniqueName, message.MessageId))
									.data;
							} catch (e) {
								vsc.window.showErrorMessage((e as AxiosError).response?.data.Message ?? e.message);
							}
	...
	private async getAssemblyFilePath(message: { fileType: string; path: string }, id?: string) {
		const file = message.path || ((await this.promptForAssemblyPath(message)) ?? "");
		if (id) {
	...
	private async saveAssembly(
		message: { IsSandboxed: boolean; DeploymentMode: number; FilePath: string },
		panelInfo: PanelInfo,
		options: { mode: "add" | "edit"; id?: string }
	): Promise<{ AssemblyName: string; Id: string } | undefined> {
		return await withAuthentication(async ({ activeOrganization }) => {
			const payload: { IsSandboxed: boolean; DeploymentMode: number; FilePath: string } = message;
			try {
			...
			try {
				return await vsc.window.withProgress(
					...
					async (_) => {
						let response: AxiosResponse<any>;
						if (options.mode === "add") {
							response = await Api.createStep(activeOrganization.UniqueName, options.pluginId!, message);
			...
			try {
				return await vsc.window.withProgress(
					...
					async (_) => {
						let response: AxiosResponse<any>;
						if (options.mode === "add") {
							response = await Api.createImage(activeOrganization.UniqueName, options.stepId!, message);
