...
using proxygenerator.Data.Builder.CS.Attributes;
using proxygenerator.Data.Builder.CS.Collections;
using proxygenerator.Data.Builder.CS.OptionSets;
...
namespace proxygenerator.Data.Builder.CS
{
    public class EntityBuilder : IEntityBuilder
    {
        private readonly EntityMetadataCache _cache;
        private readonly List<string> _availableRelatedTypes;
        private readonly bool _globalEnums;

        public EntityBuilder(EntityMetadataCache cache, List<string> availableRelatedTypes, bool globalEnums)
        {
            _cache = cache;
            _availableRelatedTypes = availableRelatedTypes;
            _globalEnums = globalEnums;
        }

        public EntityData ConstructEntity(EntityMetadata metadata)
        {
            _availableRelatedTypes.Add(metadata.LogicalName);
        ...
        private EntityData GetBaseEntity(EntityMetadata metadata)
        {
            var entityData = new EntityData
            {
                ...
                //ClassComment = new XmlSummaryComment(metadata.GetDescription())
            };
            //entityData.ClassComment.CommentParameters.Add(new XmlSummaryParameter("logicalName", entityData.LogicalName));
            //entityData.ClassComment.CommentParameters.Add(new XmlSummaryParameter("displayName", entityData.DisplayName));
            //if (metadata.IsManaged == true)
            //    entityData.ClassComment.CommentParameters.Add(new XmlSummaryParameter("managed", string.Empty));
            return entityData;
        }
        ...
        {
            var optionSetBuilder = new OptionSetBuilder(_globalEnums);
            var externalOptionSets = optionSetAttributes.Where(a => _globalEnums && a.OptionSet?.IsGlobal == true)
        ...
        private List<OptionSetData> GetInternalOptionSets(IEnumerable<EnumAttributeMetadata> optionSetAttributes)
        {
            ...
            var internalOptionSets = enumAttributes
                .Where(a => (!_globalEnums || a.OptionSet?.IsGlobal == false) &&
            ...
            var collectionFetchers = new List<CollectionFetcherData>(
                metadata.OneToManyRelationships?.Select(otm =>
                    collectionFetcherBuilder.BuildCollectionFetcher(otm,
                        _cache.GetRelatedEntity(otm.ReferencingEntity), _availableRelatedTypes))
        ...
        private List<IntersectFetcherData> GetIntersectFetchers(EntityMetadata metadata, EntityData entityData)
        {
            ...
            var fetchers = metadata.ManyToManyRelationships.Select(mtm =>
            {
                ...
                return intersectFetcherBuilder.BuildIntersectFetcher(mtm, entityData, relatedEntity, _availableRelatedTypes);
            }).ToList();
            return fetchers;
        }
