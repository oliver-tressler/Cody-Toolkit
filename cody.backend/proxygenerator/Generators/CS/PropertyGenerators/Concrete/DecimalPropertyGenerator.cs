...
{
    class DecimalPropertyGenerator : BasePropertyGenerator<PrecisionNumericAttributeData>
    {
        public DecimalPropertyGenerator(PrecisionNumericAttributeData attributeData) : base(attributeData)
        {
        }
        ...
        public override string GenerateAttributeProperties(int indentationLevel)
        {
            ...
            {
                sb.AppendLine(
                    $"{i1}set {{ this.SetPropertyValue(\"{Attribute.LogicalName}\", (decimal?)value, (decimal){Attribute.MinValue}, (decimal){Attribute.MaxValue}, \"{Attribute.CodeName}\"); }}");
            }
            sb.AppendLine($"{i}}}");
            return sb.ToString();
        }
